...
class InterfazGrafica:
    ...
    # -------------------------------------------------------------------------
    # ÁRBOL RECURSIVO (CON SCROLL X y Y)
    # -------------------------------------------------------------------------
    def dibujar_arbol_recursivo(self, nodos, x_min, x_max, y_nivel, padre_pos=None):
        ...
        for i, nodo in enumerate(nodos):
            ...
            # Líneas de conexión (X y Y con scroll)
            if padre_pos:
                mid_y = pos_y - (ESPACIO_VERTICAL_ARBOL / 2)
                pygame.draw.line(self.pantalla, COLOR_LINEA,
                                 (padre_pos[0], padre_pos[1]),
                                 (padre_pos[0], mid_y), 2)

                pygame.draw.line(self.pantalla, COLOR_LINEA,
                                 (padre_pos[0], mid_y),
                                 (punto_conexion_top[0], mid_y), 2)

                pygame.draw.line(self.pantalla, COLOR_LINEA,
                                 (punto_conexion_top[0], mid_y),
    ...
    def dibujar_interfaz(self, tablero, mensaje, tablero_raiz=None, estructura_arbol=None):
        ...
        # ------------------ ARBOL DERECHA ------------------
        titulo_arbol = self.fuente_titulo.render("Árbol de Decisiones", True, COLOR_TEXTO)
